...
if __name__ == '__main__':
    ...
    all_methods = post_hoc_attribution + ['pgexplainer', 'lri_gaussian', 'lri_bern', 'vgib'] if args.method == 'all' else [args.method]
    method_res = []
    for method_name in all_methods:
        ...
        for all_seed in args.seeds:
            ...
            # epoch_attn = (epoch_attn - epoch_attn.min()) / (epoch_attn.max() - epoch_attn.min())
            # epoch_sig_attn = (epoch_sig_attn - epoch_sig_attn.min()) / (epoch_sig_attn.max() - epoch_sig_attn.min())
            # epoch_bkg_attn = (epoch_bkg_attn - epoch_bkg_attn.min()) / (epoch_bkg_attn.max() - epoch_bkg_attn.min())
        ...
        method_dict = {'avg_attn_auc': roc_auc_score(epoch_label, avg_attn), 'std_attn_auc': roc_auc_score(epoch_label, 1-std_attn),
